...
//        DAMAGE_EFFECTS.add(PotionEffectType.HERO_OF_THE_VILLAGE);
    ...
    public static boolean isSpear(Material type) {
        return Tag.ITEMS_SPEARS.isTagged(type);
    }

    public static boolean isWaxedCopper(Material type) {
        // copied from the MaterialTags class in Paper
        return type.name().startsWith("WAXED_") && (type.name().contains("COPPER") || type.name().contains("LIGHTNING_ROD"));
    }
    ...
    public static boolean isUnwaxedCopper(Material type) {
        ...
        return switch (type) {
            case COPPER_BLOCK, CHISELED_COPPER, COPPER_DOOR, COPPER_TRAPDOOR, COPPER_GRATE, COPPER_BULB, COPPER_BARS,
                 COPPER_TORCH, COPPER_CHEST, COPPER_CHAIN, COPPER_WALL_TORCH, COPPER_LANTERN, COPPER_GOLEM_STATUE,
                 LIGHTNING_ROD -> true;
